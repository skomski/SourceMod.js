...
private:
	...
	static void CommandCallback(const CCommandContext &context, const CCommand &command);
	static void CommandCallback2(const CCommandContext &context, const CCommand &command);
	static void AddCommand(SMJS_Plugin *pl, const char *cmdName, v8::Handle<v8::Function> func, bool serverOnly);
	static void OnSayCommand_Pre(const CCommandContext &context, const CCommand &command);
	static void OnSayCommand_Post(const CCommandContext &context, const CCommand &command);
};
