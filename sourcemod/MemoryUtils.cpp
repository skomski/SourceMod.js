	...
	while (ptr < end){
		...
		for (register size_t i = 0; i < len; i++){
			if (pattern[i] != '\x2A' && pattern[i] != ptr[i]){
				...
				break;
			}else if(i != 0 && ptr[i] == pattern[0] && jumpBack == NULL){
				jumpBack = &ptr[i - 1];
			}
		}
...
int MemoryUtils::FindAndReplaceAll(const void *libPtr, size_t len, const char *pattern, const char *replace){
	int count = 0;
	...
	while (ptr < end){
		...
		if (found){
			++count;
			SourceHook::SetMemAccess(ptr, len, SH_MEM_READ | SH_MEM_WRITE | SH_MEM_EXEC);
			for (register size_t i = 0; i < len; i++){
				ptr[i] = replace[i];
			}
			found = false;
